...
def create_reservation(
    ...
    """예약 등록"""
    return crud_reservation.create_reservation(db=db, store_id=store_id, reservation=reservation)
...
def get_reservation(
    ...
    """예약 상세 조회"""
    db_reservation = crud_reservation.get_reservation(
        db=db, reservation_id=reservation_id, store_id=store_id
    )
...
def delete_reservation(
    ...
    """예약 삭제"""
    success = crud_reservation.delete_reservation(
        db=db, store_id=store_id, reservation_id=reservation_id
    )
    if not success:
        raise HTTPException(status_code=404, detail="예약을 찾을 수 없습니다")

...
from app.schemas.attendance import (
    ...
    AttendanceWithEmployee, MonthlyAttendanceSummary
)
...
def get_attendances(
    ...
    if employee_id and date:
        ...
        # 특정 날짜의 모든 직원 기록
        attendances = crud_attendance.get_attendances_by_date(db=db, attendance_date=date)
        return attendances
    elif year and month:
        # 특정 월의 모든 기록
        attendances = crud_attendance.get_attendances_by_month(db=db, year=year, month=month)
        return attendances
    else:
        raise HTTPException(
            status_code=400,
...
def get_attendance(
    ...
    """출퇴근 기록 상세 조회"""
    db_attendance = crud_attendance.get_attendance(db=db, attendance_id=attendance_id)
...
def delete_attendance(
    ...
    """출퇴근 기록 삭제"""
    success = crud_attendance.delete_attendance(db=db, attendance_id=attendance_id)
    if not success:
        raise HTTPException(status_code=404, detail="출퇴근 기록을 찾을 수 없습니다")
    return None

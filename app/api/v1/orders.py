...
def create_order(
    ...
    """카운터에서 주문 생성 (메뉴입력 완료)"""
    db_order = crud_order.create_order(db=db, store_id=store_id, order=order)
...
def get_serving_items(
    db: Session = Depends(get_db),
    store_id: int = Depends(get_store_id),
):
    """서빙 화면용: 픽업대기/조리중 아이템 목록 (order_number, table_number, order_type 포함)"""
    items = crud_order.get_order_items_for_serving(db=db, store_id=store_id)
    result = []
    for i in items:
...
def get_order_items_by_part(
    ...
    items = crud_order.get_order_items_by_part(
        db=db, store_id=store_id, part=part, statuses=statuses
    )
    result = []
    for i in items:
        d = OrderItem.model_validate(i).model_dump()
...
def get_order(
    ...
    """주문 1건 조회"""
    db_order = crud_order.get_order(db=db, order_id=order_id, store_id=store_id)
    if not db_order:
...
def update_order_item(
    ...
    if part and part_state:
        item = crud_order.update_order_item_part_status(
            db=db, store_id=store_id, item_id=item_id, part=part, part_state=part_state
        )
    else:
        item = crud_order.update_order_item_status(
            db=db, store_id=store_id, item_id=item_id, status=status
        )
    if not item:

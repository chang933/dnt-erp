...
from app.schemas.inventory_log import InventoryLog, InventoryLogCreate, InventoryLogWithIngredient
...
def create_inventory_log(
    ...
    try:
        return crud_inventory_log.create_inventory_log(db=db, log=log)
...
def get_inventory_logs(
    ...
    for log in logs:
        ...
        if ingredient:
            result.append(InventoryLogWithIngredient(
                id=log.id,
...
def get_inventory_log(
    ...
    """입출고 로그 상세 조회"""
    db_log = crud_inventory_log.get_inventory_log(db=db, log_id=log_id)
    if db_log is None:
...
def delete_inventory_log(
    ...
    """입출고 로그 삭제 (재고 자동 복구)"""
    success = crud_inventory_log.delete_inventory_log(db=db, log_id=log_id)
    if not success:
        raise HTTPException(status_code=404, detail="입출고 로그를 찾을 수 없습니다")
    return None

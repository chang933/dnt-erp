...
def _to_schema_dict(db_revenue_expense) -> Dict[str, Any]:
    rev_exp_dict = {
...
def create_revenue_expense(
    ...
    db_revenue_expense = crud_revenue_expense.create_revenue_expense(
        db=db, store_id=store_id, revenue_expense=revenue_expense
    )
    return _to_schema_dict(db_revenue_expense)
...
def get_revenue_expenses(
    ...
    return [_to_schema_dict(rev_exp) for rev_exp in db_revenue_expenses]
...
def get_revenue_summary(
    ...
    """매출/지출 요약 통계"""
    return crud_revenue_expense.get_revenue_summary(
        db=db, store_id=store_id, start_date=start_date, end_date=end_date
    )
...
def get_revenue_expense(
    ...
    """매출/지출 상세 조회"""
    db_revenue_expense = crud_revenue_expense.get_revenue_expense(
        db=db, revenue_expense_id=revenue_expense_id, store_id=store_id
    )
...
def delete_revenue_expense(
    ...
    """매출/지출 삭제"""
    success = crud_revenue_expense.delete_revenue_expense(
        db=db, store_id=store_id, revenue_expense_id=revenue_expense_id
    )
    if not success:
        raise HTTPException(status_code=404, detail="매출/지출을 찾을 수 없습니다")
    return None

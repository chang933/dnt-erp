...
from app.schemas.payroll import Payroll, PayrollCreate, PayrollUpdate, PayrollCalculate, PayrollWithEmployee
...
def calculate_and_create_payroll(
    ...
    try:
        # 급여 계산
        hourly_wage = Decimal(str(payroll_calc.hourly_wage)) if payroll_calc.hourly_wage else None
        ...
        # 급여 생성
        return crud_payroll.create_payroll(db=db, payroll=calculated_payroll)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"급여 계산 중 오류 발생: {str(e)}")


@router.get("/", response_model=List[Payroll])
def get_payrolls(
    employee_id: Optional[int] = Query(None, description="직원 ID로 필터링"),
    year_month: Optional[str] = Query(None, pattern=r'^\d{4}-\d{2}$', description="년월 (YYYY-MM 형식)"),
    ...
    if employee_id:
        ...
        # 특정 년월의 모든 급여
        payrolls = crud_payroll.get_payrolls_by_month(db=db, year_month=year_month)
    else:
        raise HTTPException(
            status_code=400,
...
def get_payrolls_by_month_with_employee(
    ...
    for payroll in payrolls:
        employee = db.query(Employee).filter(Employee.id == payroll.employee_id).first()
        if employee:
            result.append(PayrollWithEmployee(
                id=payroll.id,
...
def get_payroll(
    ...
    """급여 상세 조회"""
    db_payroll = crud_payroll.get_payroll(db=db, payroll_id=payroll_id)
...
def delete_payroll(
    ...
    """급여 삭제"""
    success = crud_payroll.delete_payroll(db=db, payroll_id=payroll_id)
    if not success:
        raise HTTPException(status_code=404, detail="급여를 찾을 수 없습니다")
    return None

...
def create_ingredient(
    ...
    """새 식자재 등록"""
    return crud_ingredient.create_ingredient(db=db, store_id=store_id, ingredient=ingredient)
...
def get_ingredients(
    ...
    """식자재 목록 조회"""
    return crud_ingredient.get_ingredients(db=db, store_id=store_id, skip=skip, limit=limit)
...
def get_ingredient(
    ...
    """식자재 상세 조회"""
    db_ingredient = crud_ingredient.get_ingredient(
        db=db, ingredient_id=ingredient_id, store_id=store_id
    )
...
def delete_ingredient(
    ...
    """식자재 삭제"""
    success = crud_ingredient.delete_ingredient(
        db=db, store_id=store_id, ingredient_id=ingredient_id
    )
    if not success:
        raise HTTPException(status_code=404, detail="식자재를 찾을 수 없습니다")
    return None

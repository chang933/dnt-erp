...
def get_next_order_number(db: Session) -> int:
    """다음 주문번호 반환 (#001, #002 ...)"""
    r = db.query(func.coalesce(func.max(Order.order_number), 0) + 1).scalar()
    return r
...
def get_order_items_by_part(db: Session, part: str, statuses: Optional[List[str]] = None) -> List[OrderItem]:
...
def get_order_items_for_serving(db: Session, status: str = "ready") -> List[OrderItem]:
    ...
    return (
        ...
        .options(joinedload(OrderItem.order))
        .join(Order)
        .filter(Order.status == "active", OrderItem.status.in_(["ready", "cooking", "pending"]))
        .order_by(OrderItem.order_id, OrderItem.created_at)
        .all()
    )


def update_order_item_status(db: Session, item_id: int, status: Optional[str] = None, part_status: Optional[dict] = None) -> Optional[OrderItem]:
    ...
    if part_status is not None:
        ...
        # 모든 파트가 ready면 전체 status를 ready로
...
def update_order_item_part_status(db: Session, item_id: int, part: str, part_state: str) -> Optional[OrderItem]:

...
def get_revenue_expense(db: Session, revenue_expense_id: int) -> Optional[RevenueExpense]:
    """매출/지출 ID로 조회"""
    return db.query(RevenueExpense).filter(RevenueExpense.id == revenue_expense_id).first()
...
def get_revenue_expenses(
    ...
    return query.order_by(desc(RevenueExpense.date), desc(RevenueExpense.id)).offset(skip).limit(limit).all()


def create_revenue_expense(db: Session, revenue_expense: RevenueExpenseCreate) -> RevenueExpense:
    """새 매출/지출 생성"""
...
def delete_revenue_expense(db: Session, revenue_expense_id: int) -> bool:
    """매출/지출 삭제"""
    db_revenue_expense = get_revenue_expense(db, revenue_expense_id)
    if not db_revenue_expense:
        return False
...
def get_revenue_summary(
    ...
    revenue_query = db.query(func.sum(RevenueExpense.amount)).filter(
        RevenueExpense.type.in_(revenue_types)
    )
    if start_date:
    ...
    expense_query = db.query(func.sum(RevenueExpense.amount)).filter(
        RevenueExpense.type.in_(expense_types)
    )
    if start_date:
    ...
    for expense_type in expense_types:
        type_query = db.query(func.sum(RevenueExpense.amount)).filter(
            RevenueExpense.type == expense_type
        )
        if start_date:

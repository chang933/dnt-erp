...
def get_inventory_logs_by_ingredient(
    ...
    ).order_by(InventoryLog.date.desc(), InventoryLog.created_at.desc()).offset(skip).limit(limit).all()
...
def get_inventory_logs_by_date_range(
    ...
    ).order_by(InventoryLog.date.desc(), InventoryLog.ingredient_id).all()


def create_inventory_log(db: Session, log: InventoryLogCreate) -> InventoryLog:
...
def delete_inventory_log(db: Session, log_id: int) -> bool:
    """입출고 로그 삭제 및 재고 복구"""
    db_log = get_inventory_log(db, log_id)
    if not db_log:
        return False
    
    # 식자재 재고 복구
    ingredient = db.query(Ingredient).filter(Ingredient.id == db_log.ingredient_id).first()
    if ingredient:

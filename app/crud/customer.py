...
def get_customer(db: Session, customer_id: int, store_id: int) -> Optional[Customer]:
    """고객 ID로 조회"""
    return (
        db.query(Customer)
...
def get_customers(
    ...
    if search:
        query = query.filter(
            or_(Customer.name.contains(search), Customer.phone.contains(search))
        )

    return query.order_by(Customer.created_at.desc()).offset(skip).limit(limit).all()


def create_customer(db: Session, store_id: int, customer: CustomerCreate) -> Customer:
    """새 고객 생성"""
...
def update_customer(
    db: Session, store_id: int, customer_id: int, customer_update: CustomerUpdate
) -> Optional[Customer]:
    """고객 정보 수정"""
    db_customer = get_customer(db, customer_id, store_id)
    if not db_customer:
        return None
...
def delete_customer(db: Session, store_id: int, customer_id: int) -> bool:
    """고객 삭제"""
    db_customer = get_customer(db, customer_id, store_id)
    if not db_customer:
        return False
